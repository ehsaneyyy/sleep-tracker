...
export default function Navbar() {
    ...
    const faceType = token
        ? idleState === "sleeping"
            ? "sleeping"
            : mode === "sleep"
                ? "tired"
                : "energetic"
        : sessionStorage.getItem("hasLoggedOut") === "true"
            ? "sad"
            : "dead";
    ...
    const buttonBg =
        faceType === "sad"
            ...
            : faceType === "dead"
                ? "bg-[#6B7280]"
                : "bg-gradient-to-br from-[#7B8CDE] to-[#A78BFA]";

    const buttonShadow =
        faceType === "sad"
            ? "hover:shadow-[0_0_16px_rgba(185,28,28,0.7)]"
            : faceType === "dead"
                ? "hover:shadow-[0_0_16px_rgba(107,114,128,0.5)]"
                : "hover:shadow-[0_0_16px_rgba(123,140,222,0.7)]";
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-8 py-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-8">
                    ...
                    <div className="relative">
                        <button
                            onClick={() => {
                                if (token) setShowAccount(!showAccount);
                            }}
                            className={`w-9 h-9 rounded-full ${buttonBg} text-[#0B0E14] flex items-center justify-center ${buttonShadow} hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer overflow-hidden`}
                        >
                            {faceType === "energetic" && (
                            ...
                            {faceType === "sleeping" && (
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" fill="none" className="w-6 h-6 relative">
                                    ...
                                    <text x="21" y="9" fill="#F1F5F9" fontSize="4.5" fontWeight="bold" className="animate-float" style={{ textShadow: "0 0 5px #7B8CDE" }}>
                                        zzz
                                    </text>
                                </svg>
                            )}
                            {faceType === "sad" && (
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" fill="none" className="w-6 h-6">
                                    <g className="animate-blink" style={{ transformOrigin: "10px 15px" }}>
                                        <circle cx="10" cy="14" r="5" fill="#F4A5A5" />
                                        <circle cx="10" cy="14" r="2" fill="white" />
                                    </g>
                                    <g className="animate-blink" style={{ transformOrigin: "26px 15px" }}>
                                        <circle cx="26" cy="14" r="5" fill="#F4A5A5" />
                                        <circle cx="26" cy="14" r="2" fill="white" />
                                    </g>
                                    <path d="M14 21 Q18 18, 22 21" stroke="#F4A5A5" strokeWidth="1.8" strokeLinecap="round" fill="none" />
                                    <circle cx="8" cy="18" r="1.8" fill="#F4A5A5" opacity="0.4" />
                                    <circle cx="28" cy="18" r="1.8" fill="#F4A5A5" opacity="0.4" />
                                </svg>
                            )}
                            {faceType === "dead" && (
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36" fill="none" className="w-6 h-6">
                                    <g className="animate-blink" style={{ transformOrigin: "10px 15px" }}>
                                        <circle cx="10" cy="14" r="5" fill="#6B7280" />
                                        <circle cx="10" cy="14" r="2" fill="white" opacity="0.4" />
                                    </g>
                                    <g className="animate-blink" style={{ transformOrigin: "26px 15px" }}>
                                        <circle cx="26" cy="14" r="5" fill="#6B7280" />
                                        <circle cx="26" cy="14" r="2" fill="white" opacity="0.4" />
                                    </g>
                                    <path d="M14 21 Q18 21, 22 21" stroke="#6B7280" strokeWidth="1.8" strokeLinecap="round" fill="none" />
                                </svg>
                            )}
                        </button>
